...
# exclui comanda
@router.delete("/comanda/{numero}", status_code=status.HTTP_204_NO_CONTENT, tags=["Comanda"], summary="Remover comanda - protegida por JWT e grupo 1")
...
async def delete_comanda(
    ...
    try:
        # Buscar a comanda
        result = await db.execute(
            select(ComandaDB).where(ComandaDB.comanda == numero)
        )
        comanda = result.scalar_one_or_none()
        ...
        result = await db.execute(
            ...
            .where(ComandaDB.comanda == numero)
        )
...
# cancelar comanda
@router.put("/comanda/{numero}/cancelar", response_model=ComandaResponse, tags=["Comanda"], summary="Cancelar comanda - protegida por JWT e grupo 1")
@limiter.limit("critical")
...
@router.post("/comanda/{numero}/produto", response_model=ComandaProdutosResponse, status_code=status.HTTP_201_CREATED, tags=["Comanda"], summary="Adicionar produto à comanda - protegida por JWT")
@limiter.limit("restrictive")
async def add_produto_to_comanda(
    ...
    try:
        ...
        result = await db.execute(
            select(ProdutoDB).where(ProdutoDB.id == produto_data.produto_id)
        )
...
@router.get("/comanda/{numero}/produtos", response_model=List[ComandaProdutosResponse], tags=["Comanda"], summary="Listar produtos de uma comanda - protegida por JWT")
@limiter.limit("moderate")
...
async def update_comanda_produto(
    ...
    try:
        ...
        result = await db.execute(
            select(ComandaProdutoDB).where(ComandaProdutoDB.id == id)
        )
