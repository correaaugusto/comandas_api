...
from src.domain.Schemas.AuthSchema import ClienteAuth
...
# Dependency para validar token e retornar cliente atual
def get_current_cliente(
    credentials: HTTPAuthorizationCredentials = Depends(security),
    db: Session = Depends(get_db)
) -> ClienteAuth:
    """Dependency que valida o token e retorna o cliente atual"""

    # Extrai e valida o token
    payload = verify_access_token(credentials.credentials)
    cpf: str = payload.get("sub")
    id_cliente: int = payload.get("id")

    if cpf is None or id_cliente is None:
        raise HTTPException(
    ...
    cliente = db.query(ClienteDB).filter(ClienteDB.id == id_cliente).first()
    if not cliente:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
